...
export default function AdminTeams() {
  ...
  const availableForSubs = usersWithoutTeam.filter(
    (user) => !subs.some((sub) => sub.userId === user.id),
  );

  const availableUsersForEdit = users.filter((user) => {
    return (
      !usersIdsOnTeams.includes(user.id) ||
      editingTeam?.users.some((u) => u.id === user.id)
    );
  });
  ...
  return (
    <div>
          ...
          <div key={division.id}>
            ...
            {division.teams.map((team) => (
              <Flex align="center" gap="2" key={team.id}>
                ...
                <IconButton
                  variant="ghost"
                  onClick={() => editTeam(team as unknown as TeamWithUsers)}
                >
                  <Pencil2Icon />
                </IconButton>
              </Flex>
      ...
      {isEditing && editingTeam ? (
        <Form method="post" ref={editingFormRef} key={navigation.state}>
          <Card>
            <Heading>Edit team users</Heading>
            <Flex gap="3" py="3" direction="column">
              <Select.Root
                key={editingTeam?.users[0].id}
                name="player1"
                defaultValue={editingTeam?.users[0].id || ""}
              >
              ...
              <Select.Root
                key={editingTeam?.users[1].id}
                name="player2"
                defaultValue={editingTeam?.users[1].id || ""}
              >
