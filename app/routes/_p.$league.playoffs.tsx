      ...
      <Card>
        <Flex>
          {rounds.map((round, roundIndex) => (
            <div
              key={roundIndex}
              className="flex-1 flex flex-col justify-around gap-y-8"
            >
              {Array.from({ length: round.games }).map((_, gameIndex) => {
                ...
                return (
                  <Flex direction="column" key={gameIndex} className="relative">
                    {(() => {
                      const match = league.playoffMatches.find(
                        (m) => m.bracketNumber === gameNumber,
                      );
                      return (
                        <>
                          <Text
                          ...
                          <Text
                            size="1"
                            className={`p-2 ${round.paddingTop} border-b border-r border-gray-600 whitespace-nowrap`}
                          >
                            {match?.teams[1]
                              ? getTeamName(
